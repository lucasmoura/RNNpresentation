...
\visible<8->{
\node[textonly, right=80pt of activemc] (alulco) {Least Confident};
\path[tedge] (activemc) edge node[above=1.8pt] {} (alulco.west);
}

\visible<7->{
\node[textonly, above=10pt of alulco.west, anchor=west] (alumc) {Active Learning + MC Dropout};
\path[tedge] (activemc) edge node[above=1.8pt] {} (alumc.west);
}

\visible<9->{
\node[textonly, below=10pt of alulco.west, anchor=west] (aluh) {Entropy};
\path[tedge] (activemc) edge node[above=1.8pt] {} (aluh.west);
}

\visible<10->{
\node[textonly, below=10pt of aluh.west, anchor=west] (alumi) {Mutual Information};
...
\visible<14->{
\node[textonly, right=80pt of actives] (alslco) {Least Confident};
\path[tedge] (actives) edge node[above=1.8pt] {} (alslco.west);
}

\visible<13->{
\node[textonly, above=10pt of alslco.west, anchor=west] (alsmc) {Active Learning + Softmax};
\path[tedge] (actives) edge node[above=1.8pt] {} (alsmc.west);
}

\visible<15->{
\node[textonly, below=10pt of alslco.west, anchor=west] (alsh) {Entropy};
\path[tedge] (actives) edge node[above=1.8pt] {} (alsh.west);
}

\visible<16->{
\node[textonly, below=10pt of alsh.west, anchor=west] (alsmi) {Mutual Information};
